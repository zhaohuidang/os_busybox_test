...
pub struct Fat<T> {
    /// Cache manager for fat，fat表的cache管理器
    fat_cache_mgr: Arc<Mutex<T>>,
    /// The first block id of FAT.第一张fat表的起始磁盘编号
    /// In FAT32, this is equal to bpb.rsvd_sec_cnt
    start_block_id: usize,
    /// size fo sector in bytes copied from BPB，每个扇区有多少字节
    byts_per_sec: usize,
    /// The total number of FAT entries，一张fat表里一共有多少个簇
    tot_ent: usize,
    /// The queue used to store known vacant clusters，记录那些被释放的簇的簇号，可用于下次申请。
    vacant_clus: Mutex<VecDeque<u32>>,
    /// The final unused cluster id we found，优化参数，遍历fat表申请空簇时可以直接从hint开始而不用从头开始，减少遍历时间。
    hint: Mutex<usize>,
}
