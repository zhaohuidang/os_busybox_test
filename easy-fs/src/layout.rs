...
impl BPB {
    ...
    pub fn is_valid(&self) -> bool {
        self.root_dir_sec() == 0
            && self.tot_sec16 == 0//对于fat32,此域必须为0
            && self.count_of_cluster() >= 66625 /*May not apply to the REAL WORLD/test*/
            //数据区的簇数量，count_of_cluster()是对函数data_sector_count()的包装，关于此函数，在下面有介绍
            && self.fat_sz16 == 0//对于fat32,此域必须为0
            && self.root_ent_cnt == 0
    }
    #[inline(always)]
    //计算数据扇区数量
    pub fn data_sector_count(&self) -> u32 {
        self.tot_sec32
            - (self.rsvd_sec_cnt as u32
                ...
                + self.root_dir_sec())
                //公式为：总扇区数-（保留区的扇区数+fat表数量*fat32表所占的扇区数+根目录的扇区数）
    }
    ...
    pub fn count_of_cluster(&self) -> u32 {
        ...
        //公式为：扇区数/每簇的扇区数，得到簇的数量
    }
    #[inline(always)]
    /// The size of cluster counted by the sectors.计算每簇的字节数，即簇的大小
    pub fn clus_size(&self) -> u32 {
        (self.byts_per_sec * (self.sec_per_clus as u16)) as u32
        //计算公式：每个扇区的字节数*每簇的扇区数
    }
   ...
   //计算公式：（（（根目录的目录项数（长度为32bytes）*32）+（每个扇区的字节数-1））/每个扇区的字节数
    }
    #[inline(always)]
