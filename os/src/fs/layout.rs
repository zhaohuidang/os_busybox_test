...
bitflags! {
    pub struct OpenFlags: u32 {
        const O_RDONLY      =   0o0;//只读模式
        const O_WRONLY      =   0o1;//只写模式
        const O_RDWR        =   0o2;//可读可写
        
	//以下的常量是选用的，这些选项是用来和上面的必选项进行按位或起来作为flags参数。
        const O_CREAT       =   0o100;//表示如果指定文件不存在，则创建这个文件
        const O_EXCL        =   0o200;//表示如果要创建的文件已存在，则出错，同时返回 -1，并且修改 errno 的值。
        const O_NOCTTY      =   0o400;//如果路径名指向终端设备，不要把这个设备用作控制终端。
        const O_TRUNC       =   0o1000;//表示截断，如果文件存在，并且以只写、读写方式打开，则将其长度截断为0。
	
        const O_APPEND      =   0o2000;//表示追加，如果原来文件里面有内容，则这次写入会写在文件的最末尾。
        const O_NONBLOCK    =   0o4000;//如果路径名指向 FIFO/块文件/字符文件，则把文件的打开和后继 I/O设置为非阻塞模式（nonblocking mode）
        
        //以下三个常量同样是选用的，它们用于同步输入输出
        const O_DSYNC       =   0o10000;//等待物理 I/O 结束后再 write。在不影响读取新写入的数据的前提下，不等待文件属性更新。
        const O_SYNC        =   0o4010000;//等待物理 I/O 结束后再 write，包括更新文件属性的 I/O
        const O_RSYNC       =   0o4010000;//等待所有写入同一区域的写操作完成后再进行
        
        const O_DIRECTORY   =   0o200000;//不是目录
        const O_NOFOLLOW    =   0o400000;//如果 pathname 参数指向的是一个符号链接，将不对其进行解引用，直接返回错误。
