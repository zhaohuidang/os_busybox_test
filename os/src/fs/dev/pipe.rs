...
impl File for Pipe {
    ...
    fn read(&self, offset: Option<&mut usize>, buf: &mut [u8]) -> usize {
        ...
        loop {
            ...
            //只要读不空，就一直读
                let read_bytes = ring.buffer_read(&mut buf[buf_start..]);//获取缓冲区读数据的大小
                buf_start += read_bytes;//更新开始读的位置
                read_size += read_bytes;//更新已读数据的大小
                if ring.head == ring.tail {
                //读空了
                    ring.status = RingBufferStatus::EMPTY;//设置缓冲区状态
                    read_size += buf_start;
                    return read_size;
                }
            }
